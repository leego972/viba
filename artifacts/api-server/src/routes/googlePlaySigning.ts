...
router.post("/play-publisher/apps/:id/signing", async (req, res): Promise<void> => {
  try {
    await schema();
    ...
    if (!keystoreBase64 || !storePassword || !keyAlias || !keyPassword) {
      send(res, 400, { error: "Keystore file, store password, alias and key password are required" });
      return;
    }
    ...
    if (bytes.length < 100 || bytes.length > 5 * 1024 * 1024) {
      send(res, 400, { error: "Invalid keystore size" });
      return;
    }

    const fingerprint = createHash("sha256").update(bytes).digest("hex");
    await pool.query(
      `INSERT INTO play_publisher_signing(user_id,app_id,encrypted_keystore,encrypted_store_password,key_alias,encrypted_key_password,certificate_sha256)
       VALUES($1,$2,$3,$4,$5,$6,$7)
       ON CONFLICT(user_id,app_id) DO UPDATE SET
         encrypted_keystore=EXCLUDED.encrypted_keystore,
         encrypted_store_password=EXCLUDED.encrypted_store_password,
         key_alias=EXCLUDED.key_alias,
         encrypted_key_password=EXCLUDED.encrypted_key_password,
    ...
    send(res, 400, { error: error instanceof Error ? error.message : "Signing setup failed" });
  }
});

router.get("/play-publisher/apps/:id/signing", async (req, res): Promise<void> => {
  try {
    await schema();
    const result = await pool.query(
      "SELECT key_alias,certificate_sha256,updated_at FROM play_publisher_signing WHERE app_id=$1 AND user_id=$2",
      [Number(req.params.id), uid(req)],
    );
    res.json(result.rows[0] ? { configured: true, ...result.rows[0] } : { configured: false });
  } catch (error) {
    send(res, 500, { error: error instanceof Error ? error.message : "Signing lookup failed" });
  }
});
