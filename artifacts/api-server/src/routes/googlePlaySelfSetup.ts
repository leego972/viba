...
router.post("/play-publisher/self-setup", async (req, res): Promise<void> => {
  try {
    ...
    const { connectionId, privacyPolicyUrl = "https://viba.guru/privacy" } = req.body as {
      connectionId?: number;
      privacyPolicyUrl?: string;
    };

    if (!connectionId) {
      res.status(400).json({ error: "Connect your Google Play account first" });
      return;
    }

    const connection = await pool.query(
      "SELECT id FROM play_publisher_connections WHERE id=$1 AND user_id=$2",
      [connectionId, userId],
    );
    if (!connection.rows[0]) {
      res.status(404).json({ error: "Google Play connection not found" });
      return;
    }
    ...
    const result = await pool.query(
      `INSERT INTO play_publisher_apps
       ...
       ON CONFLICT(user_id,package_name) DO UPDATE SET
         ...
         updated_at=NOW()
       RETURNING *`,
      [userId, connectionId, privacyPolicyUrl],
    );
