...
router.post("/api/production-ops/targets", async (req, res): Promise<void> => {
  ...
  const { appName, publicUrl, apiHealthUrl = "", railwayProjectId, railwayServiceId, strictMode = false } =
    req.body as { appName?: string; publicUrl?: string; apiHealthUrl?: string; railwayProjectId?: string; railwayServiceId?: string; strictMode?: boolean };
  ...
  const { rows } = await pool.query<Record<string, unknown>>(
    `INSERT INTO viba_production_targets (user_id, app_name, public_url, api_health_url, railway_project_id, railway_service_id, strict_mode, status)
     VALUES ($1,$2,$3,$4,$5,$6,$7,'unknown') RETURNING *`,
    [u, appName, publicUrl, apiHealthUrl, railwayProjectId ?? null, railwayServiceId ?? null, Boolean(strictMode)],
  );
