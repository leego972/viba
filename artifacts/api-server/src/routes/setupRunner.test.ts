import { describe, it, expect, vi, afterEach } from "vitest";
  ...
  describe("POST /setup/apply", () => {
    ...
    it("rejects without X-Admin-Confirm header (requireConfirmation not bypassed)", async () => {
      const app = await makeApp();
      // TEST_BYPASS_ADMIN=1 skips requireAdmin, but requireConfirmation is never bypassed
    ...
    it("defaults replace to false and skipDeploys to true", async () => {
      ...
      vi.spyOn(globalThis, "fetch").mockImplementationOnce(async (_url, opts) => {
        ...
        capturedInput = parsed.variables?.input ?? null;
      ...
      if (capturedInput) {
        expect(capturedInput.replace).toBe(false);
        expect(capturedInput.skipDeploys).toBe(true);
      }
      delete process.env.RAILWAY_TOKEN;
    });
    ...
    it("unapproved variable names are stripped from Railway mutation", async () => {
      ...
      if (capturedInput && typeof capturedInput.variables === "object" && capturedInput.variables !== null) {
        expect(Object.keys(capturedInput.variables as object)).not.toContain("EVIL_KEY");
      }
      delete process.env.RAILWAY_TOKEN;
    });
  });
