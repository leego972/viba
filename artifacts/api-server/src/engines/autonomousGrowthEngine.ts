...
export async function updateAutonomousGrowthSettings(input: {
  enabled?: boolean;
  intervalMinutes?: number;
  freeChannels?: string[];
  autoPublishChannels?: string[];
  maxPiecesPerCycle?: number;
...
export async function getAutonomousGrowthStatus() {
  ...
  const autoApproveThreshold = Math.max(0, Math.min(100, Number(settings["growth_auto_approve_threshold"] ?? 78)));
  ...
  return {
    ...
    notes: [
      ...
      "LinkedIn, X/Twitter, Reddit and YouTube Shorts generate approved content queues unless API credentials are present and a publisher is added.",
    ],
  };
}
...
async function ensureMarketingContentForChannels(channels: string[], maxPieces: number) {
  ...
  const blastChannels = channels
    .map((c) => (c === "x_twitter" ? "twitter" : c))
    .filter((c) => ["linkedin", "twitter", "reddit", "devto", "discord"].includes(c));
  ...
  if (channels.includes("blog")) {
    const content = await generateCreatorContent({
      ...
      topic: "How VIBA coordinates multiple AI models in one workflow",
      campaignObjective: "organic SEO and founder-led education",
      seoKeywords: ["multi-agent AI orchestration", "AI workflow automation", "connect ChatGPT Claude Gemini"],
    });
  ...
  if (channels.includes("youtube_shorts")) {
    const content = await generateCreatorContent({
      ...
      topic: "Why one AI model is not enough for serious project work",
      campaignObjective: "educate developers and founders",
      seoKeywords: ["AI agents", "multi-agent AI", "AI orchestration"],
    });
    const [inserted] = await db.insert(marketingContent).values({
      platform: "youtube_shorts",
      type: "video_script",
...
async function publishToDevTo(item: typeof marketingContent.$inferSelect): Promise<PublishResult> {
  ...
  const tags = Array.isArray(item.hashtags)
    ? item.hashtags.map((tag) => tag.replace(/^#/, "").toLowerCase()).filter(Boolean).slice(0, 4)
  ...
  const res = await fetch("https://dev.to/api/articles", {
    ...
    body: JSON.stringify({
      article: {
        title: item.headline ?? "VIBA: Multi-Agent AI Orchestration",
        published: true,
        tags,
...
async function publishContentItem(item: typeof marketingContent.$inferSelect, autoPublishChannels: string[]): Promise<PublishResult> {
  ...
  if (!autoPublishChannels.includes(platform)) {
    return { contentId: item.id, platform, status: "queued", reason: "Channel not enabled for auto-publish" };
  }
...
export async function publishApprovedFreeContent(limit = 20, autoPublishChannels = DEFAULT_AUTO_PUBLISH_CHANNELS): Promise<PublishResult[]> {
  const rows = await db.select().from(marketingContent)
    .where(eq(marketingContent.status, "approved"))
...
export async function processCreatorSchedulesWithPublishing() {
  const due = await db.select().from(contentCreatorSchedules)
    .where(and(eq(contentCreatorSchedules.status, "pending"), lt(contentCreatorSchedules.scheduledAt, new Date())));
...
export async function runAutonomousGrowthCycle(source = "manual"): Promise<GrowthCycleResult> {
  if (growthRunning) {
    return growthLastResult ?? { success: false, generated: 0, published: 0, queued: 0, failed: 0, results: [], message: "Growth cycle already running" };
  }
  ...
  try {
    ...
    await runAutonomousContentCycle({ maxPiecesPerPlatform: Math.min(4, maxPiecesPerCycle), autoApproveThreshold, autoSchedule: true });
