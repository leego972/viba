...
/**
 * Spec alias: assert that an action is approved, throwing if it is not.
 * Delegates to assertApproved with a pending-outcome guard.
 */
export function assertApproval(
  ...
  assertApproved(action, outcome, decision);
}

/**
 * Spec: build a structured approval request object describing what needs approval.
 */
export function buildApprovalRequest(
  action: string,
  context: ApprovalContext = {},
): ApprovalRequest {
  const decision = requiresApproval(action, context);
  const highRisk: ApprovalCategory[] = ["repository_write", "destructive_operation", "deployment_change", "provider_key_change", "out_of_scope", "domain_dns_change"];
  const isHigh = decision.categories.some(c => highRisk.includes(c));
  const isMed = !isHigh && decision.required;
  return {
    approvalRequired: decision.required,
    reason: decision.reasons[0] ?? "Approval required for this action",
    requiredPhrase: isHigh ? "I approve this VIBA action" : undefined,
