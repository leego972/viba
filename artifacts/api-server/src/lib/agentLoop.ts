...
export async function runNextAgentStep(sessionId: number): Promise<{
  ...
  if (result.blockedReason && !assignedAgent.canUseTools) {
    ...
    await logAudit(sessionId, "tool_handoff", `${assignedAgent.name} blocked — tool handoff to capable agent`, { originalTaskId: nextTask.id, siblingTaskId: siblingTask.id, blockedReason: result.blockedReason });
  ...
  if (result.toolOutputs && result.toolOutputs.length > 0) {
    ...
    await logAudit(sessionId, "tool_outputs_persisted", `${assignedAgent.name} produced ${result.toolOutputs.length} tool output(s)`, { taskId: nextTask.id, agentId: assignedAgent.id, outputTypes: result.toolOutputs.map((o) => o.type) });
  }
  ...
  await logAudit(sessionId, "agent_message_generated", `${assignedAgent.name} completed task: ${nextTask.title}`, { taskId: nextTask.id, agentId: assignedAgent.id });
  ...
  if (questionMessages.length > 0) {
    await logAudit(sessionId, "agent_questions_sent", `${assignedAgent.name} sent ${questionMessages.length} question(s)`, { taskId: nextTask.id, questionCount: questionMessages.length });
  }
