...
export class MistralAdapter implements AgentAdapter {
  ...
  constructor(id: string, name: string, role: string, apiKey: string, model?: string, canUseTools = false) {
    ...
    this.role = role;
    this.apiKey = apiKey;
    this.model = model ?? process.env["MISTRAL_MODEL"] ?? "mistral-large-latest";
...
${buildAdapterJsonSchema(this.canUseTools, input.pendingQuestions, this.canUseTools)}`;
    ...
    try {
      ...
      const inputCost = ((usage?.prompt_tokens ?? 0) / 1_000_000) * 2.00;
      const outputCost = ((usage?.completion_tokens ?? 0) / 1_000_000) * 6.00;
      const cost = inputCost + outputCost;

      return parseAdapterJson(text, cost);
    } catch (err) {
      logger.error({ err }, "Mistral API call failed");
      throw err;
    }
  }
  ...
  async evaluateTask(goal: string, peers: Array<{ name: string; role: string }>): Promise<{ accepted: boolean; reason?: string }> {
    try {
      const { default: OpenAI } = await import("openai");
      const client = new OpenAI({ apiKey: this.apiKey, baseURL: MISTRAL_BASE_URL, timeout: 10_000 });
