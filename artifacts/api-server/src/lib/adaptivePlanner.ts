...
// ─── Spec-compliant generateAdaptivePlan output ───────────────────────────────
...
export interface SpecAdaptivePlan {
  goal: string;
  planId: string;
  steps: SpecPlanStep[];
...
function stepRisk(needsApproval: boolean, type: string): "low" | "medium" | "high" {
  if (needsApproval) return "high";
  if (type === "deployment" || type === "repair") return "medium";
  return "low";
}

/**
 * Generate a spec-compliant adaptive plan with planId, steps, stop conditions,
 * and success criteria. Wraps the existing classifyAndPlan internals.
 */
export function generateAdaptivePlan(goal: string, routerResult?: { recommendedProvider?: string }): SpecAdaptivePlan {
  const internal = buildAdaptivePlan(goal);
  const planId = `plan-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
  const provider = routerResult?.recommendedProvider ?? "groq";

  const steps: SpecPlanStep[] = internal.tasks.map((t: PlannedTask, i: number) => {
    const prevId = i > 0 ? `step-${i}` : undefined;
    return {
      id: `step-${i + 1}`,
      title: t.title,
      actionType: t.type,
      provider: t.requiredTools.some((tool: string) => tool.startsWith("github.")) ? "github" : provider,
      tool: t.requiredTools[0],
      dependsOn: prevId ? [prevId] : [],
      risk: stepRisk(t.needsApproval, t.type),
      approvalRequired: t.needsApproval,
      expectedEvidence: t.requiredTools.length > 0
        ? t.requiredTools.map((tool: string) => `output of ${tool}`)
        : [`completion confirmation for: ${t.title}`],
    };
  });

  const stopConditions = [
    "Owner explicitly cancels the plan",
    "Three consecutive steps fail with no recoverable path",
    "Security vulnerability detected that requires immediate halt",
  ];

  const successCriteria: string[] = [];
  for (const output of internal.expectedReportOutput) {
    successCriteria.push(`Evidence collected: ${output.replace(/_/g, " ")}`);
  }
  successCriteria.push("All approval checkpoints cleared by owner");
  successCriteria.push("Final review step completed without blockers");

  const reviewStep = internal.approvalCheckpoints[0];
  const nextReviewPoint = reviewStep
    ? `After step ${reviewStep.afterStep}: ${reviewStep.reason}`
    : "After all steps complete — owner final review";

  return { goal, planId, steps, stopConditions, successCriteria, nextReviewPoint };
}

// ─── Spec-required export (T08) ──────────────────────────────────────────────

export interface CreateAdaptivePlanInput {
  goal: string;
  valueRoute?: {
...
/**
 * Generate a step-by-step adaptive execution plan.
 * Spec-required entry point — wraps generateAdaptivePlan with the spec input/output contract.
 */
export function createAdaptivePlan(input: CreateAdaptivePlanInput): SpecAdaptivePlan {
  return generateAdaptivePlan(input.goal, {
    recommendedProvider: input.valueRoute?.recommendedProvider,
  });
}
