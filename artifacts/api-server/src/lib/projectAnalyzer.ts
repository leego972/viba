...
export function analyzeProject(input: AnalyzerInput): ProjectAnalysis {
  ...
  } else if (fileList.some((f) => f.includes("vercel.json") || f.includes(".vercelignore"))) {
    deploymentTarget = "vercel";
  } else if (fileList.some((f) => f.includes("render.yaml") || f.includes("render.yml"))) {
    deploymentTarget = "render";
  } else if (fileList.some((f) => f.includes(".do/app.yaml") || f.includes(".do/deploy.template.yaml"))) {
    deploymentTarget = "digitalocean";
  } else if (fileList.some((f) => f.includes("Dockerfile") || f.includes("docker-compose.yml") || f.includes("docker-compose.yaml"))) {
  ...
  // User-selected provider always wins over file-detected hint
  if (input.deploymentProvider) {
    const chosen = input.deploymentProvider.toLowerCase();
    deploymentTarget = chosen;
    if (chosen === "railway") {
      railwayReadiness = envMissing.length === 0 ? "ready" : "needs_config";
    }
  }
