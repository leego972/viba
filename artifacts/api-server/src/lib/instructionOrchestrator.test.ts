...
describe("planInstruction", () => {
  ...
  it("uses current web research only when the instruction requires it", () => {
    ...
    const local = planInstruction("Write a summary of the current project context");
  ...
  it("recognises UX and UI at the beginning of an instruction", () => {
    expect(planInstruction("UX review of the dashboard").tasks.some((task) => task.type === "creative_direction")).toBe(true);
    expect(planInstruction("UI redesign for onboarding").tasks.some((task) => task.type === "creative_direction")).toBe(true);
  });

  it("does not treat incidental substrings as task keywords", () => {
    const plan = planInstruction("Summarise the address and explain the result");
    expect(plan.tasks).toHaveLength(1);
    expect(plan.tasks[0]?.type).toBe("planning");
  });

  it("does not mark a normal sentence complex merely because it contains also", () => {
    expect(planInstruction("Write the title and also a subtitle").complexity).toBe("simple");
  });
