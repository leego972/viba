...
export default function PlayPublisherPage() {
  ...
  const createConnection = useMutation({ mutationFn: (body: any) => api("/api/play-publisher/connections", { method: "POST", body: JSON.stringify(body) }), onSuccess: () => { setMessage("Your Google Play connection was verified and encrypted."); refresh(); }, onError: e => setMessage(e.message) });
  ...
  return <main className="min-h-screen bg-background text-foreground p-4 md:p-8">
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div><p className="text-xs uppercase tracking-[.25em] text-muted-foreground">Mobile Publishing</p><h1 className="text-3xl font-semibold">VIBA Play Publisher</h1><p className="mt-1 text-sm text-muted-foreground">Audit, build, sign and publish Android applications using each customer’s own Google account.</p></div>
      ...
      {tab === "overview" && <section className="grid gap-5 lg:grid-cols-2">
        <Panel title="Release safeguards"><ul className="space-y-2 text-sm text-muted-foreground"><li>• Every user supplies their own Google service-account credentials.</li><li>• VIBA has no owner or platform Google credential fallback.</li><li>• Credentials are isolated by authenticated user and encrypted with AES-256-GCM.</li><li>• Production releases require explicit approval.</li><li>• Artifact SHA-256 is verified before upload.</li><li>• All release commits are audit logged.</li></ul></Panel>
        <Panel title="Required server configuration"><Code>PLAY_PUBLISHER_MASTER_KEY=&lt;strong-random-secret&gt;</Code><p className="mt-3 text-sm text-muted-foreground">This key only encrypts customer-supplied credentials. It is not a Google credential and cannot publish by itself.</p></Panel>
      </section>}
...
function ConnectionForm({ submit, busy }: { submit: (v:any)=>void; busy:boolean }) {
  const [name,setName]=useState("Google Play");
  const [serviceAccountJson,setJson]=useState("");
  const [confirmedOwnAccount,setConfirmed]=useState(false);
  return <Panel title="Connect your Google Play service account"><div className="space-y-3">
    <div className="rounded-md border bg-muted/40 p-3 text-sm text-muted-foreground">Use a service account created inside your own Google Cloud project and authorised in your own Google Play Console. VIBA will never use the VIBA owner’s Google credentials for your app.</div>
    <Field value={name} onChange={e=>setName(e.target.value)} placeholder="Connection name"/>
    <textarea value={serviceAccountJson} onChange={e=>setJson(e.target.value)} className="min-h-56 w-full rounded-md border bg-background p-3 font-mono text-xs" placeholder="Paste your service-account JSON"/>
    <label className="flex items-start gap-3 text-sm"><input type="checkbox" checked={confirmedOwnAccount} onChange={e=>setConfirmed(e.target.checked)} className="mt-1"/><span>I confirm these credentials belong to my own Google Cloud project and Google Play Console account, and I authorise VIBA to use them only for my applications.</span></label>
    <button disabled={busy || !confirmedOwnAccount || !serviceAccountJson.trim()} onClick={()=>submit({name,serviceAccountJson,confirmedOwnAccount})} className="rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-50">{busy?"Verifying…":"Verify and connect my account"}</button>
  </div></Panel>;
}
