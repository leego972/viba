...
export default function ProjectImportPage() {
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      if (deploymentProvider) body["deploymentProvider"] = deploymentProvider;
    ...
    setCreating(false);
  }, [sourceType, repoUrl, description, knownErrors, strictMode, deploymentProvider, toast]);
      ...
      <div className="container max-w-5xl py-8 space-y-6">
        ...
        <Card className="border-white/[0.07] bg-white/[0.03]">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-1">
              <label className="text-xs text-foreground/50">Deployment Provider (optional)</label>
              <select
                className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary/40"
                value={deploymentProvider}
                onChange={(e) => setDeploymentProvider(e.target.value)}
              >
                <option value="">Auto-detect from project files</option>
              ...
              {deploymentProvider === "sevall" && (
                <p className="text-[10px] text-yellow-400">Sevall support is available as a manual-guided provider until API integration is confirmed.</p>
              )}
            </div>
