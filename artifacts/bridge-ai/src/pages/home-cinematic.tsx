...
const SCENES: Array<{ id: Scene; label: string; detail: string; duration: number }> = [
  ...
  { id: "planning", label: "Planning", detail: "Breaking the audit into specialist tasks", duration: 3000 },
  { id: "disconnect", label: "Issue detected", detail: "A critical pathway stopped responding", duration: 2500 },
  { id: "repair", label: "Repairing", detail: "Rerouting work through an available pathway", duration: 2900 },
  { id: "verify", label: "Verifying", detail: "Checking the repair against evidence and tests", duration: 2800 },
  { id: "complete", label: "Audit complete", detail: "One consolidated release-readiness result", duration: 3200 },
];
      ...
      <main>
        <section className="viba-hero viba-concise-hero" id="audit">
          <div className="viba-hero-copy">
            ...
            <p className="viba-hero-lead">Paste a GitHub repository. V.I.B.A. coordinates specialist AIs to inspect, test and rank the production blockers in one verified report.</p>
        ...
        <section className="viba-incident-visual" aria-label="Failure scenarios">
          <div><span>SEE V.I.B.A. RESPOND</span><h2>Choose a failure scenario.</h2></div>
          <div className="viba-incident-deck">{INCIDENTS.map((item, index) => <button key={item.id} type="button" className={`viba-incident-button ${index === incidentIndex ? "is-active" : ""}`} onClick={() => selectIncident(index)}><small>SCENARIO {String(index + 1).padStart(2, "0")}</small><strong>{item.label}</strong><i /></button>)}</div>
        </section>
