  ...
  return (
    <div className={`viba-network viba-scene-${scene}`} aria-label={`VIBA orchestration visualisation: ${scene}. ${incident.label}`}>
      ...
      <div className="viba-brain-shell">
        ...
        <div className="viba-core-caption"><span>V.I.B.A.</span><small>{scene === "complete" ? "UNIFIED OUTPUT" : isBusy ? "ORCHESTRATING" : "READY"}</small></div>
      </div>
      ...
      {NODES.map((node, index) => {
        ...
        return (
          <div key={node.id} className={`viba-agent-node ${affected ? "is-affected" : ""} ${restored ? "is-restored" : ""}`} style={{ left: `${node.x}%`, top: `${node.y}%`, animationDelay: `${index * -0.7}s` }}>
            <div className="viba-node-ring" />
            <div className="viba-node-dot">{affected ? <IncidentIcon /> : restored ? <RefreshCw /> : <Sparkles />}</div>
            <span>{node.label}</span><small>{affected ? "INTERRUPTED" : restored ? "RESTORED" : isBusy ? "ACTIVE" : "STANDBY"}</small>
          </div>
        );
      })}
      ...
      <main>
        <section className="viba-hero" id="orchestration">
          <div className="viba-hero-copy">
            ...
            <div className="viba-incident-deck" aria-label="Failure scenarios">{INCIDENTS.map((item, index) => <button key={item.id} type="button" className={`viba-incident-button ${index === incidentIndex ? "is-active" : ""}`} onClick={() => selectIncident(index)}><small>SIMULATE {String(index + 1).padStart(2, "0")}</small><strong>{item.label}</strong><i /></button>)}</div>
