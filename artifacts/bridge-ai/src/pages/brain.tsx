...
export default function BrainPage() {
  ...
  const fetchBrain = useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      const [capRes, invRes, routeRes, providerRes] = await Promise.all([
      ...
      if (!capRes.ok) setError("Capability route is not mounted yet. The visual shell works, but Replit must wire /api/tools/capabilities for full live status.");
    } catch {
        ...
        <div className="relative mx-auto max-w-screen-2xl px-4 py-6 space-y-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Link href="/tool-console"><Button variant="secondary" className="gap-2"><Code2 className="h-4 w-4" /> Toolbox</Button></Link>
            </div>
          </div>
