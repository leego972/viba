...
const WORKFLOWS: WorkflowDefinition[] = [
  {
    ...
    label: "Repair",
    icon: Wrench,
    prompt: "Diagnose, plan, verify, and produce evidence before declaring success.",
  },
  {
    type: "design",
    label: "Design",
    icon: Eye,
    prompt: "Improve clarity, visual hierarchy, trust, mobile layout, and conversion.",
  },
  {
    type: "upgrade",
    label: "Upgrade",
    icon: Sparkles,
    prompt: "Improve architecture, reliability, safety gates, user experience, and monitoring.",
  },
  {
    type: "deploy",
    label: "Deploy",
    icon: Zap,
    prompt: "Check readiness, provider setup, verification, rollback notes, and evidence.",
  },
];
...
function stageForTool(toolId: string): { label: string; icon: IconComponent } {
  ...
  return (
    <div className="relative mx-auto flex h-64 w-64 items-center justify-center">
      ...
      <div className="relative z-10 flex h-28 w-28 flex-col items-center justify-center rounded-full border border-white/10 bg-black/40 shadow-2xl backdrop-blur-xl">
        <Brain className={`h-9 w-9 ${brainColor}`} />
...
export default function ToolConsolePage() {
  ...
  const fetchBrain = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (capRes.ok) {
        ...
        setCapabilities([]);
        setError("Capability route is not mounted yet. The screen works as a visual shell, but Replit must wire /api/tools/capabilities for full live status.");
      }
  ...
  const metrics: MetricCardDefinition[] = [
    { label: "Executable", value: counts.executable ?? 0, icon: CheckCircle2 },
    { label: "Planning", value: counts.planning_only ?? 0, icon: Brain },
    { label: "Credentials", value: counts.credential_required ?? 0, icon: ShieldCheck },
    { label: "Need work", value: warnings, icon: Wrench },
        ...
        <div className="relative mx-auto max-w-screen-2xl px-4 py-6 space-y-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.3em] text-primary">
                ...
                Live AI command surface
              </div>
              <h1 className="mt-2 text-3xl font-bold tracking-tight md:text-5xl">The VIBA Brain</h1>
              <p className="mt-2 max-w-3xl text-sm text-muted-foreground md:text-base">
                A captivating operator screen where users can watch agent work, follow the tool route, see proof status, and interrupt at any time.
              </p>
            </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[360px_1fr_360px]">
            <Card className="border-white/[0.06] bg-black/20 backdrop-blur-xl">
              ...
              <CardContent>
                <BrainOrb active={(counts.executable ?? 0) + (counts.planning_only ?? 0)} warnings={warnings} />
                <div className="mt-4 text-center text-xs text-muted-foreground">
                  {tools.length} registered tools · {capabilities.length} classified capabilities
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-white/[0.06] bg-black/20 backdrop-blur-xl">
              ...
              <CardContent className="grid gap-4 lg:grid-cols-[1fr_240px]">
                ...
                <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
                  ...
                  <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{selected.prompt}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-white/[0.06] bg-black/20 backdrop-blur-xl">
              ...
              <CardContent className="space-y-3">
                <Textarea
                  ...
                  placeholder="Tell VIBA what to change, pause, avoid, or prioritise…"
                ...
                <p className="text-xs text-muted-foreground">
                  Replit should wire these controls into the active session message and stop APIs.
                </p>
              </CardContent>
            </Card>
          </div>
            ...
            <Card className="border-white/[0.06] bg-black/20 backdrop-blur-xl">
              ...
              <CardContent className="space-y-2">
                {invocations.length === 0 ? (
                  <p className="py-6 text-center text-sm text-muted-foreground">No recent tool activity yet.</p>
                ) : (
                  invocations.slice(0, 8).map((item, index) => (
                    <div key={item.id ?? index} className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.025] p-3">
            ...
            <Card className="border-white/[0.06] bg-black/20 backdrop-blur-xl">
              ...
              <CardContent className="grid gap-2 md:grid-cols-2">
                {capabilities.length === 0 ? (
                  <p className="col-span-full py-6 text-center text-sm text-muted-foreground">No capability data yet.</p>
                ) : (
                  capabilities.slice(0, 10).map((item) => <CapabilityCard key={item.toolId} item={item} />)
                )}
              </CardContent>
            </Card>
          </div>
