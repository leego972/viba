...
export default function ProductionOpsPage() {
  ...
  const handleAddTarget = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${BASE}/api/production-ops/targets`, {
        ...
        body: JSON.stringify({ appName, publicUrl, apiHealthUrl, railwayProjectId: railwayProjectId || undefined, providerId, strictMode }),
      });
      ...
      setAppName(""); setPublicUrl(""); setApiHealthUrl(""); setRailwayProjectId(""); setProviderId("railway");
      await fetchTargets();
  ...
  }, [appName, publicUrl, apiHealthUrl, railwayProjectId, providerId, strictMode, toast, fetchTargets, fetchOpsSummary]);
              ...
              <div className="border border-white/[0.08] rounded-lg p-4 space-y-3 bg-white/[0.02]">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="md:col-span-2">
                    <label className="text-[10px] text-foreground/40 uppercase tracking-wide">Deployment Provider</label>
                    <select className="mt-1 w-full bg-white/[0.04] border border-white/[0.08] rounded px-3 py-1.5 text-sm text-foreground focus:outline-none focus:border-primary/40" value={providerId} onChange={(e) => setProviderId(e.target.value)}>
                      <option value="railway">Railway</option>
                      <option value="render">Render</option>
                      <option value="digitalocean">DigitalOcean</option>
                      <option value="vercel">Vercel</option>
                      <option value="sevall">Sevall</option>
                      <option value="custom">Custom / Other</option>
                    </select>
                    {providerId === "sevall" && (
                      <p className="mt-1 text-[10px] text-yellow-400">Sevall support is available as a manual-guided provider until API integration is confirmed.</p>
                    )}
                  </div>
                </div>
