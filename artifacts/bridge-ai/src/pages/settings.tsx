...
const PROVIDERS: ProviderConfig[] = [
  ...
  },
  {
    ...
    placeholder: "...",
    hint: "Powers Mistral build/code-review agents. Get your key at console.mistral.ai.",
    providerName: "mistral",
    modelKey: "MISTRAL_MODEL",
    defaultModel: "mistral-large-latest",
    models: [
      { value: "mistral-large-latest", label: "Mistral Large (default)" },
  ...
  },
  {
    ...
    placeholder: "sk-...",
    hint: "Powers DeepSeek research/reasoning agents. Get your key at platform.deepseek.com.",
  ...
  },
  {
    ...
    placeholder: "venice-...",
    hint: "Privacy-first uncensored AI. Get your key at venice.ai. Uses the OpenAI-compatible endpoint at api.venice.ai automatically.",
    providerName: "venice",
    modelKey: "VENICE_MODEL",
    defaultModel: "llama-3.3-70b",
    models: [
      { value: "llama-3.3-70b",     label: "Llama 3.3 70B (default)" },
      { value: "llama-3.1-405b",    label: "Llama 3.1 405B" },
      { value: "mistral-31-24b",    label: "Mistral 31 24B" },
  ...
  },
  {
    ...
    hint: "Any OpenAI-compatible API — OpenRouter, Together AI, a self-hosted vLLM, etc. Enter the base URL below and a model name above.",
    providerName: "custom",
    ...
    endpointHint: "Base URL of the OpenAI-compatible API (e.g. https://openrouter.ai/api/v1).",
  },
  {
    key: "GITHUB_TOKEN",
...
const ALL_SETTING_KEYS = [
  ...PROVIDERS.flatMap((p) => {
    const keys = [p.key];
    if (p.modelKey) keys.push(p.modelKey);
    if (p.endpointKey) keys.push(p.endpointKey);
    return keys;
  }),
  ...ALERT_KEYS,
  ...SMTP_KEYS,
          ...
          <CardContent className="space-y-6">
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} id="settings-form">
              {isLoading ? (
                ...
                PROVIDERS.map(({ key, label, placeholder, hint, providerName, modelKey, defaultModel, models, endpointKey, endpointPlaceholder, endpointHint, modelFreeText }) => {
                    ...
                    >
                      ...
                      {modelKey && modelFreeText && (
                        <div className="flex items-center gap-2">
                          ...
                          <Input
                            ...
                            name={modelKey}
                            type="text"
                            placeholder="e.g. gpt-4o, meta-llama/llama-3.3-70b-instruct"
                      ...
                      {endpointKey && (
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <Label htmlFor={endpointKey} className="text-xs text-muted-foreground whitespace-nowrap flex items-center gap-1">
                            ...
                            <Input
                              ...
                              name={endpointKey}
                              type="url"
                              placeholder={endpointPlaceholder ?? "https://api.example.com/v1"}
                              value={values[endpointKey] ?? ""}
                              onChange={handleKeyChange}
                              className="flex-1 h-8 text-xs font-mono"
                            />
                          </div>
                          {saveResults[endpointKey] === "saved" && (
                            <p className="flex items-center gap-1 text-xs text-emerald-500 pl-20">
                              <CheckCircle2 className="h-3 w-3" /> URL saved
                            </p>
                          )}
                          {endpointHint && (
                            <p className="text-xs text-muted-foreground pl-20">{endpointHint}</p>
                          )}
                        </div>
                      )}
