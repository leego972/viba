...
import { Pin, PinOff, Trash2, Plus, Loader2, AlertTriangle, CheckCircle, Brain } from "lucide-react";
...
export default function ProjectMemoryPage() {
  ...
  const { data: memoriesData, isLoading } = useQuery<{ memories: ProjectMemoryEntry[] }>({
    ...
    queryFn: () => fetch(`/api/project-memory?projectId=${encodeURIComponent(selectedProject)}`, { credentials: "include" }).then(r => r.json()),
    enabled: !!selectedProject,
  });

  const addMemory = useMutation({
    mutationFn: () => fetch("/api/project-memory", {
  ...
  const patchMemory = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["/api/project-memory"] }),
  });

  const deleteMemory = useMutation({
    mutationFn: (id: number) => fetch(`/api/project-memory/${id}`, { method: "DELETE", credentials: "include" }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/project-memory"] }); toast({ title: "Memory deleted" }); },
  });
      ...
      <div className="container max-w-4xl py-8 space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Structured context stored once and reused — instead of re-sending full conversation history.
          </p>
        </div>
        ...
        <Card className="border-border/50">
          ...
          <CardContent className="space-y-3">
                ...
                <SelectContent>
                  {projects.map(p => (
                    <SelectItem key={p.project_id} value={p.project_id}>
                      {p.project_id} <span className="text-muted-foreground ml-2">({p.memory_count} entries)</span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="flex gap-2">
              <Input
                placeholder="Or enter a new project ID…"
                value={newProjectId}
                onChange={e => setNewProjectId(e.target.value)}
                className="h-9 text-sm"
              />
              <Button size="sm" variant="outline" onClick={() => { if (newProjectId) { setSelectedProject(newProjectId); setNewProjectId(""); } }}>
                Load
              </Button>
            </div>
          </CardContent>
          ...
          <>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{memories.length} memory entries</p>
              </div>
            ...
            {showAdd && (
              <Card className="border-primary/30 bg-primary/5">
                ...
                <CardContent className="space-y-3">
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <Select value={newEntry.memoryType} onValueChange={v => setNewEntry(e => ({ ...e, memoryType: v }))}>
                        ...
                        <SelectContent>
                          {MEMORY_TYPES.map(t => <SelectItem key={t} value={t} className="capitalize">{t}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                    <div>
                      <Label className="text-xs mb-1.5 block">Key / label</Label>
                      <Input placeholder="e.g. tech_stack" value={newEntry.key} onChange={e => setNewEntry(n => ({ ...n, key: e.target.value }))} className="h-9 text-sm" />
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => addMemory.mutate()} disabled={!newEntry.key || !newEntry.value || addMemory.isPending}>
            ...
            {isLoading ? (
              ...
              <Card className="border-border/50">
                <CardContent className="py-16 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">No memory entries yet. Add context that VIBA should reuse instead of re-sending it every time.</p>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-4">
                {pinned.length > 0 && (
                  <div>
                    <p className="text-xs text-muted-foreground font-medium mb-2">Pinned</p>
                    <div className="space-y-2">
                      {pinned.map(m => <MemoryCard key={m.id} memory={m} onPatch={patchMemory.mutate} onDelete={deleteMemory.mutate} />)}
                    </div>
                  </div>
                )}
                {unpinned.length > 0 && (
                  <div>
                    {pinned.length > 0 && <p className="text-xs text-muted-foreground font-medium mb-2">Other entries</p>}
                    <div className="space-y-2">
                      {unpinned.map(m => <MemoryCard key={m.id} memory={m} onPatch={patchMemory.mutate} onDelete={deleteMemory.mutate} />)}
                    </div>
                  </div>
                )}
              </div>
...
function MemoryCard({ memory: m, onPatch, onDelete }: {
  memory: ProjectMemoryEntry;
  onPatch: (data: Record<string, unknown>) => void;
  onDelete: (id: number) => void;
}) {
  return (
    <Card className={`border-border/40 transition-colors ${m.pinned ? "border-primary/20 bg-primary/5" : "bg-card/50"}`}>
      <CardContent className="py-3 px-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed line-clamp-3">{m.value}</p>
